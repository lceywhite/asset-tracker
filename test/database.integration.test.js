...
test("upgrades v4 plans, separates check sessions and completes a backup round-trip", async () => {
  ...
  assert.equal(migratedPlan.modelVersion, 3)
  assert.equal(migratedPlan.startsAt, "2026-07-14")
  assert.equal(migratedPlan.endsAt, "2026-07-14")
  assert.ok(migratedPlan.packingItems[0].id)
  assert.ok(migratedPlan.packingItems[0].containerId)
